 ...
 * Ucigame uses many kinds of sprites: they can be stationary; they can
 * cycle between multiple images, and they can act like buttons and be
 * sensitive to mouse clicks. Sprites can have text written on them, and
 * multiple sprites can be "pinned" together so that when one moves the
 * others follow.
 * 
 * @author Dan Frost
 */
public class Sprite
{
	...
	/**
	 * This constructor takes an already loaded Image object, and creates a
	...
	/**
	 * This makeSprite method takes an already loaded Image object, and
	 * creates a Sprite with the specified width and height. If width is
	 * larger than image's width, and/or height is larger than image's
	 * height, then the image will be tiled to cover the complete sprite.
	 */
	 ...
	 * canvas with draw(), it will automatically cycle to the next image.
	 * Image is an already loaded Image object. The x and y parameters
	 * specify the upper left hand corner of a rectangle in the image that
	 * will be sprite's next frame. The width and height of that rectangle
	...
	/**
	 * The method takes a frame from image (with an upper left hand corner
	 * as specified by x and y and with width and height as specified in
	 * makeTiledSprite()), and puts that frame into one or more of
	 * sprite1's tiles as specified by the col and row pairs. Note that
	 * counting columns and rows starts from zero, and that position (0, 0)
	 * is in the matrix's upper right hand corner. This method can only be
	 * used if sprite1 was created with makeTiledSprite.
	 */
	...
	/**
	 * Pins the sprite (sprite2) passed in on top of current sprite
	 * (sprite1), which means that sprite2 will be drawn whenever
	 * sprite1.draw() is performed. The upper left hand corner of sprite2
	 * is located x pixels to the right and y pixels below the upper left
	 * hand corner of sprite1; thus sprite2 moves when sprite1 is moved.
	 * Any number of other sprites can be pinned to a sprite. A sprite
	 * cannot be pinned to itself. If a sprite is hidden, all sprites
	 * pinned to it are hidden.
	 */
	...
	/**
	 * Sets a framerate for this sprite; Ucigame will try to change the
	 * sprite's current frame number times per second, although there is no
	 * guarantee that it will be successful. This method has no effect if
	 * number is negative or greater than the framerate specified for the
	 * entire game.
	 */
	...
	/**
	 * Rotates the sprite counterclockwise by the number of degrees
	 * specified. The center of rotation is the center of the sprite. Any
	 * sprites pinned to this sprite are also rotated around the same
	 * center of rotation. Note that rotation occurs immediately before
	 * sprite.draw() is executed, no matter when the call to rotate() is
	 * made (as long as rotate() is called before draw()). This means that
	 * rotation does not affect collision detection; collision detection is
	 * computed on the unrotated sprite. (Defined as a feature, but could
	 * well be a bug.) After draw() is called on a sprite any rotation is
	 * removed; thus if a sprite should be rotated in every frame rotate()
	 * must be called before each draw().
	 */
	...
	/**
	 * Same as the one parameter version of rotate(), except that the
	 * center of rotation is x pixels to the right and y pixels down from
	...
	/**
	 * This methods flip the sprite around a vertical line running through
	...
	/**
	 * This methods flip the sprite around a horizontal line running
	...
	/**
	 * Causes the sprite, and any other sprites pinned to this sprite, to
	 * be drawn on the canvas, unless the sprite is hidden. Can only be
	 * called inside the game class's draw() method.
	 */
	public void draw() {
	...
	/**
	 * Draws the specified int on the sprite, using the sprite's current
	 * font. The lower left hand corner of text is located x pixels to the
	 * right and y pixels below the upper left hand corner of sprite1. Any
	 * part of text that lies outside the sprite is clipped (not
	 * displayed).
	 */
	...
	/**
	 * Draws the specified text on the sprite, using the sprite's current
	 * font. The lower left hand corner of text is located x pixels to the
	 * right and y pixels below the upper left hand corner of sprite1. Any
	 * part of text that lies outside the sprite is clipped (not
	 * displayed).
	...
	/**
	 * Moves the sprite so that its upper left hand corner is at the
	 * specified position, relative to the upper left hand corner of the
	 * canvas.
	 */
	...
	/**
	 * Moves the sprite by its x and y motion amounts. Typically this
	 * method is called once for every moving sprite, before collision
	 * detection.
	 */
	public void move() {
	...
	/**
	 * This method return the x values of the sprite's upper left hand
	 * corner (relative to the upper left hand corner of the canvas).
	 * 
	...
	/**
	 * This method return the y values of the sprite's upper left hand
	 * corner (relative to the upper left hand corner of the canvas).
	 * 
	 ...
	 * corner of the canvas). This method is usually used instead of
	 * move(), but like move() is called before collision detection and
	 * before draw(). Calling this method sets the sprites x motion amounts
	 * to 0.
	 * 
	 * @param _nextX
	 ...
	 * corner of the canvas). This method is usually used instead of
	 * move(), but like move() is called before collision detection and
	 * before draw(). Calling this method sets the sprites y motion amounts
	 * to 0.
	 * 
	 * @param _nextY
	...
	/**
	 * Returns a boolean value of true or false, depending on whether an
	 * collision was detected in the immediately preceeding
	...
	/**
	 * This method tests whether this sprite overlaps with the sprite
	 * passed in and bounce if collision occurs.
	 */
	...
	/**
	 * This method tests whether this sprite overlaps with the sprite
	 * passed in and stop movement if collision occurs.
	 */
	...
	/**
	 * Like the other xxxIfCollidesWith() methods, this methods test
	 * whether the two sprites will overlap in the next x and y positions.
	 * However, this methods don't change the position or motion of either
	 * sprite. The programmer can then test whether an overlap occurred
	 * with collided() and code the desired behavior.
	 */
	...
	/**
	 * This method tests whether this sprite overlaps with the sprite
	 * passed in and pauses. Pausing means that sprite1 moves as far as
	 * possible without overlapping sprite2, and its x and y motion amounts
	 * are unchanged.
	 */
