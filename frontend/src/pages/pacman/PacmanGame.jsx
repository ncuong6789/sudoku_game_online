...
export default function PacmanGame() {
    ...
    const initGame = useCallback(() => {
        ...
        setGhosts(GHOST_STARTS.map(g=>({...g, startX:g.x, startY:g.y,
        ...
        <div style={{
            ...
            <div style={{position:'relative',height:'100%',maxHeight:'100%',aspectRatio:`${cols}/${rows}`,maxWidth:'100%',flexShrink:0}}>
                <div style={{
                    ...
                    {Array.from(pills).map(k=>{const[x,y]=k.split(',').map(Number);return(
                ...
                {(phase==='over'||phase==='won')&&(
                    <div style={{position:'absolute',inset:0,background:'rgba(13,17,23,0.95)',backdropFilter:'blur(8px)',
            ...
            {/* ── SIDEBAR ── */}
            <div style={{display:'flex',flexDirection:'column',gap:'clamp(0.4rem,0.8vw,0.8rem)',
