...
export function useChessLogic(mode, roomId, difficulty, initialColor, callbacks) {
    ...
    const hintSuggestionsRef = useRef([]);
        ...
        try {
            ...
            stockfishWorker.onmessage = (e) => {
                ...
                if (line === 'readyok') {
                    ...
                    if (engineTask.current === 'eval' || engineTask.current === 'move') {
                    ...
                    } else if (engineTask.current === 'hint') {
                        const scoreMatch = line.match(/score cp (-?\d+)/);
                        const pvMatch = line.match(/pv\s+([a-h][1-8][a-h][1-8])/);
                        if (scoreMatch && pvMatch && hintSuggestionsRef.current.length < 5) {
                            const score = parseInt(scoreMatch[1]);
                            const moveStr = pvMatch[1];
                            const move = { from: moveStr.substring(0, 2), to: moveStr.substring(2, 4) };
                            if (!hintSuggestionsRef.current.find(s => s.from === move.from && s.to === move.to)) {
                                hintSuggestionsRef.current.push({ ...move, score });
                            }
                        }
                    }
                } else if (line.startsWith('bestmove')) {
                    if (engineTask.current === 'eval' || engineTask.current === 'hint_stopping') {
                        return; // Ignore bestmove generated by 'stop' or 'eval'
                    }
                    if (engineTask.current === 'hint') {
                        ...
                        if (moveMatch) {
                            ...
                            const suggestions = hintSuggestionsRef.current;
                        ...
                        if (moveMatch && mode === 'solo') {
                            ...
                            setGame((g) => {
                                ...
                                try {
                                    ...
                                    if (result) {
                                        if (result.flags.includes('c')) callbacks.onCapture?.();
                                        else callbacks.onMove?.();
                                        if (newGame.isCheck()) callbacks.onCheck?.();
                                    }
                                    return newGame;
                                } catch (err) { return g; }
                            });
    ...
    const getHint = useCallback(() => {
        ...
        setHintMove(null);
        setHintSuggestions(null);
        hintSuggestionsRef.current = [];

        engineTask.current = 'hint_stopping';
        engine.current.postMessage('stop'); // Stop any ongoing analysis first
        
        setTimeout(() => {
            if (engineTask.current === 'hint_stopping') {
                engineTask.current = 'hint';
                engine.current.postMessage(`position fen ${game.fen()}`);
                engine.current.postMessage(`go movetime 1500`);
            }
        }, 50);
    }, [game, isEngineReady, gameOver, myColor]);
